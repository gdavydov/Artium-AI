...
// see canManageCollection() below; the resolver's guard is the real
// enforcement point, this is just the query it relies on.
//
...
// import { CollectionsService, CollectionInput, PeriodInput } from './collections.service';
...
//
//   // Admin/Curator always; Contributor only with a `manage` CollectionAccess grant
//   @UseGuards(JwtAuthGuard)
//   @Mutation(() => Collection)
//   async createCollection(@Args('input') input: CollectionInput, @CurrentUser() user) {
//     if (user.role === 'contributor') throw new ForbiddenException();
//     return this.collections.create(input, user.displayName);
//   }
//
//   @UseGuards(JwtAuthGuard)
//   @Mutation(() => Collection)
...
//     @Args('id') id: string,
//     @Args('input') input: Partial<CollectionInput>,
//     @CurrentUser() user,
//   ) {
//     const allowed = await this.collections.canManageCollection(user.id, user.role, id);
//     if (!allowed) throw new ForbiddenException();
//     return this.collections.update(id, input, user.displayName);
//   }
//
//   @UseGuards(JwtAuthGuard)
//   @Mutation(() => Period)
...
//     @CurrentUser() user,
//   ) {
//     const allowed = await this.collections.canManageCollection(user.id, user.role, collectionId);
//     if (!allowed) throw new ForbiddenException();
